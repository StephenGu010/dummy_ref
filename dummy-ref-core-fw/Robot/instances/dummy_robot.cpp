...
inline float ClampFloat(float _val, float _min, float _max)
{
...
void DummyRobot::SetLedEnable(bool _enable)
{
    isLedEnabled = _enable;
}

bool DummyRobot::GetLedEnabled()
{
    return isLedEnabled;
}
...
void DummyRobot::MarkCompliantCmdRx()
{
    compliantLastRxMs = millis();
    compliantActive = true;
}

void DummyRobot::CompliantWatchdogTick()
{
    if (commandMode != COMMAND_COMPLIANT_CURRENT || !isEnabled || !compliantActive)
        return;

    // Safety: auto zero currents if host stream timeout.
    if (millis() - compliantLastRxMs > COMPLIANT_TIMEOUT_MS)
        ZeroJointCurrents();
}
...
void DummyRobot::SetRGBColor(uint8_t _r, uint8_t _g, uint8_t _b)
{
    rgbColor[0] = _r;
    rgbColor[1] = _g;
    rgbColor[2] = _b;
...
void DummyRobot::GetRGBColor(uint8_t &_r, uint8_t &_g, uint8_t &_b)
{
    _r = rgbColor[0];
    _g = rgbColor[1];
    _b = rgbColor[2];
...
uint32_t DummyRobot::GetCommandMode()
{
    return static_cast<uint32_t>(commandMode);
}

const char* DummyRobot::GetCommandModeShortName(uint32_t mode)
{
    switch (mode)
    {
        ...
        case COMMAND_MOTOR_TUNING:
            return "MOTOR_TUNE";
        case COMMAND_COMPLIANT_CURRENT:
            return "COMP_CURRENT";
        default:
            return "UNKNOWN";
    }
...
void DummyRobot::SetCommandMode(uint32_t _mode)
{
    if (_mode < COMMAND_TARGET_POINT_SEQUENTIAL ||
        _mode > COMMAND_COMPLIANT_CURRENT)
        return;

    CommandMode nextMode = static_cast<CommandMode>(_mode);

    if (commandMode == COMMAND_COMPLIANT_CURRENT &&
        nextMode != COMMAND_COMPLIANT_CURRENT)
    {
        ZeroJointCurrents();
        compliantActive = false;
    }
...
uint32_t DummyRobot::CommandHandler::ParseCommand(const std::string &_cmd)
{
    ...
    switch (context->commandMode)
    {
        ...
        case COMMAND_COMPLIANT_CURRENT:
            ...
            {
                float currents[6];
                argNum = sscanf(_cmd.c_str(), "$%f,%f,%f,%f,%f,%f",
                                currents, currents + 1, currents + 2,
                ...
                } else
                {
                    ...
                    Respond(*uart4StreamOutputPtr, "error BAD_CURRENT_CMD");
                }
            }
            break;
    }
