...
void ThreadOledUpdate(void* argument)
{
    uint32_t t = micros();
    char buf[16];
    const char cmdModeNames[5][6] = {"SEQ", "INT", "TRJ", "TUNE", "CURR"};
...
void ThreadRGBUpdate(void* argument) {
    ...
    for (;;) {
        pwm.SetDuty(PWM::CH_A1, dummy.GetLedEnabled() ? 0.5f : 0.0f);

        if (dummy.GetRGBEnabled())
        {
            uint32_t mode = dummy.GetRGBMode();
            if (mode == RGB::CUSTOM_COLOR)
            {
                dummy.GetRGBColor(rgbR, rgbG, rgbB);
                rgb.SetCustomColor(rgbR, rgbG, rgbB);
                rgb.Run(RGB::CUSTOM_COLOR);
            } else
            {
                rgb.Run((RGB::Rgb_style_t) mode);
            }
            osDelay(30);
        }else
        {
